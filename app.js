const KEY='md2a083',$=id=>document.getElementById(id),C=MD2.classes;
...
">${j<s.levelCursor?'✓':j===s.levelCursor?'●':'○'} ${heroSpoken(s.heroes[e.i])} — ${e.status}</div>`).join('')+'</div>'}}
...
$('confirmGroup').onclick=()=>{if(!s.heroes.length)return alert('Añade al menos un héroe.');if(s.mode==='solo'&&s.heroes.length!==1)return alert('El modo solitario requiere exactamente 1 héroe.');if(s.mode==='coop'&&s.heroes.length<2)return alert('El modo cooperativo requiere al menos 2 héroes.');s.confirmed=true;s.active=0;s.resurrection={blue:s.heroes.length<=2?1:s.heroes.length<=4?2:3,grey:0};save();render();enqueueSpeech(s.mode==='solo'?'Modo Solitario activado. Tu héroe dispone de 4 acciones por turno. La vida de monstruos errantes y la cantidad de secuaces se resuelven como una partida de 2 héroes. Ahora debes elegir la habilidad inicial.':'Grupo confirmado. Ahora debes elegir la habilidad inicial de cada héroe.',h(),advancePending)};
$('playerMode').onchange=e=>{if(s.confirmed)return alert('El modo no puede cambiarse después de confirmar el grupo.');s.mode=e.target.value;if(s.mode==='solo'&&s.heroes.length>1)s.heroes=s.heroes.slice(0,1);s.heroes.forEach(x=>x.actions=s.mode==='solo'?4:3);save();renderSetup()};
...
$('exportSettings').onclick=()=>{let b=new Blob([JSON.stringify(s,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(b);a.download='md2_alpha083.json';a.click()};
